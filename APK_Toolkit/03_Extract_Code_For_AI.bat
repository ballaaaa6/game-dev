...
rem Reuse a valid caller path when 00_OneClick_Auto_Extract supplies one.
rem The script is also copied into *_Dumped, so fall back from one level to
rem two levels above this file to find Ghidra beside the workspace root.
...
if not defined GHIDRA_MAX_PAYLOAD_MB set "GHIDRA_MAX_PAYLOAD_MB=256"
...
echo [INFO] Max decompiler payload: !GHIDRA_MAX_PAYLOAD_MB! MB
